...
-- Classic does not have access to the TooltipInfo API unfortunately
...
local function IsItemAccountbound(text)
    if not text then return end
    if text:GetText() == ITEM_BIND_TO_BNETACCOUNT or text:GetText() == ITEM_ACCOUNTBOUND then
        return true
    end
    return false
end
...
function CIMIScanTooltip:IsItemWarbound(itemLink, bag, slot)
    ...
    if bag and slot then
        ...
        return self:ScanTooltipBreak(IsItemAccountbound, itemLink)
    end
end